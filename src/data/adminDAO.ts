...
export default class adminDAO {
    ...
    async updateOrganization(org: Organization) {
        ...
        const sqlUpdate = 'UPDATE organization SET name=$1, image=$2, info=$3, main_color=$4, approval_status=$5 WHERE id = $6 RETURNING *';
        try {
            client = await db.connect()
    ...
    async createAdmin(admin: Admin, organizationId: string) 
    {
        ...
        const sqlCreate = "INSERT INTO admin (auth_id, first_name, last_name, language, role, status, organization_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *";

        try {
            client = await db.connect()
            let results = await client.query(sqlCreate, [
                admin.getAuthId(),
                admin.getFirstName(),
                admin.getLastName(),
                admin.getLanguage(),
    ...
    async updateAdmin(admin: Admin) {
        ...
        const sqlUpdate = 'UPDATE admin SET first_name=$1, last_name=$2, language=$3, role=$4, status=$5 WHERE auth_id = $6 RETURNING *';
        try {
            client = await db.connect()
            let response = await client.query(sqlUpdate, [
                admin.getFirstName(),
...
let admin = new Admin("test1", "Noah", "Roerig", "ENGLISH", "noahr1936@gmail.com", "MASTER",new Date(), "VALID")
