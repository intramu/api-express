...
router.get("/players", async (req, res) => {
    ...
    const response = await organizationService.findAllPlayersByAdminId(sub);
...
router.get("/teams", async (req, res) => {
    const { sub = "" } = req.auth?.payload ?? {};
    const response = await organizationService.findAllTeamsByAdminId(sub);
...
router.patch("/teams/:teamId", teamIdParam, async (req, res) => {
    ...
    const response = await teamService.patchTeam(team);
    return handleErrorResponse(response, res);
});
